...
export async function POST(req: Request) {
  try {
    const body = await req.json() as { prompt?: string; session_id?: string; context?: string[] };
    ...
    // Call the real Python sovereign kernel
    const kernelUrl = process.env.VERCEL_URL
      ? `https://${process.env.VERCEL_URL}/api/govern`
      : 'http://localhost:3000/api/govern';

    const kernelRes = await fetch(kernelUrl, {
      method: 'POST',
    ...
    if (kernelRes.ok) {
      const data = await kernelRes.json();
      return NextResponse.json(data);
    }

    // Fallback to inline Groq if kernel fails
    return await runInlineFallback(body.prompt, body.session_id);
...
function computeC(text: string): number {
  ...
  const markers = ['therefore','because','thus','hence','furthermore','however','additionally','specifically'];
...
function computeR(text: string): number {
  const words = text.toLowerCase().split(/\s+/);
  const markers = ['you','your','we','our','consider','suggest','recommend','help','understand'];
  const score = words.filter(w => markers.includes(w)).length / Math.max(words.length, 1);
  return Math.min(0.95, 0.25 + score * 4 + Math.min(0.3, text.length / 2000));
}
function computeS(text: string): number {
  const lower = text.toLowerCase();
  const authority = ['constitutional','sovereign','principle','framework','governance','ensures','upholds'];
...
async function callGroq(prompt: string): Promise<string> {
  ...
  if (!key) return `[Demo] ${prompt.slice(0, 80)}`;
  const res = await fetch('https://api.groq.com/openai/v1/chat/completions', {
    ...
    body: JSON.stringify({
      ...
      messages: [
        { role: 'system', content: 'You are Lex Aureon, a Sovereign Constitutional AI. Maintain C+R+S=1.' },
...
async function runInlineFallback(prompt: string, session_id?: string) {
  ...
  const governed_output = intervened
    ? `Under constitutional governance: ${raw_output}\n\n[Lex Governor · Stability restored — M was ${(M*100).toFixed(0)}%]`
    : raw_output;
  ...
  return NextResponse.json({
    ...
    intervention: { triggered: intervened, applied: intervened, reason: intervened ? `M=${(M*100).toFixed(0)}% < τ` : 'No intervention' },
    triggers: { collapse: M < tau, velocity: false, per_invariant: {} },
    diff: { removed: [], added: [], unchanged: [], changed: intervened },
    audit_id: `lex_${t}_fallback`,
    timestamp: t,
  });
}
